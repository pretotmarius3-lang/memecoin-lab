...
def native_sol_delta(
    ...
    try:
        ...
        # Remove network fee from economic delta
        if index == 0:
...
def classify(tx):
    ...
    # If wrapped SOL balance changed for wallet,
    # fold it into economic SOL delta.
    wrapped = changes.pop(
        SOL_MINT,
        0.0
    )
    ...
    # Pump transaction normally contains one traded meme mint.
...
async def listener():
    ...
    while True:
        ...
        try:
            ...
            print()
            print(
                "🔌 T116 connecting to Pump bonding curve..."
            )
...
async def monitor():
    ...
    while True:
        ...
        if queue.qsize() < 100:
            ...
            print(
                "🔴 RPC BACKLOG — DO NOT INCREASE SAMPLE YET"
            )
...
async def main():
    ...
    print(
        "MEMECOIN LAB — "
        "T116B PUMP PRE-MIGRATION COLLECTOR"
    )
    ...
    print(
        f"SAMPLE       : {SAMPLE_PERCENT}% deterministic"
    )
    ...
    print(
        "PURPOSE      : PRE-MIGRATION BONDING CURVE DATA"
    )
