...
Consumes V5.4.2 admission tiers. If V5.4.4 is running, its learned A/B/C/D
budget is loaded dynamically; otherwise a conservative static budget is used.
Scheduling is by RPC attempt share, not success share.
"""
from __future__ import annotations
import asyncio, json, os, signal, sqlite3, time
import aiohttp
...
def init():
    ...
    print(f'V5.4.3 SCIENCE FETCHER | provider={base.KEY_SOURCE} target={RPS:.1f}rps tiers=A/B/C/D adaptive_budget=enabled',flush=True)
...
def learned_budget():
    ...
    try:
        ...
        if total<=0:return dict(STATIC_BUDGET),'STATIC'
        return {k:b.get(k,0)/total for k in 'ABCD'},'LEARNED'
    finally:c.close()
...
def emit(started,attempts,states,inserted,rps,budget,mode):
    ...
    print(f"V5.4.3 | {mode} plan[{plan}] pace={rps:.2f} ok={okps:.2f}/s attempts={total} OK={ok} ERR={err} NULL={states.get('NULL',0)} 429={states.get('429',0)} dup={states.get('DUP',0)} inserted={inserted} pending={pending:,} raw={raw:,} served[{mix}] available[{avail}]",flush=True)
...
async def main():
    ...
    async with aiohttp.ClientSession(connector=connector,headers={'Content-Type':'application/json'}) as session:
        while not STOP:
            ...
            if row is None:
                # Prefer an available lane with the highest current learned budget.
