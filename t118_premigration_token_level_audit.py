...
# This is DATA QUALITY control, not a trading threshold.
# Extremely large 300s returns are retained in the DB/report
# but excluded from descriptive/model-oriented statistics.
#
# +1000% in 300s is deliberately very permissive.
# We also reject mathematically impossible loss below -100%.
...
def path_quality(r):
    ...
    n = r["path_snapshots_300s"]

    if n is None or n < 1:
        return "NO_DATA"

    mx = r["path_max_return_300s"]
    mn = r["path_min_return_300s"]
    end = r["path_end_return_300s"]
...
def show():
    ...
    print()
    print("=" * 180)
    print("3 — PATH QUALITY / EXTREME RETURN AUDIT")
    print("=" * 180)

    first_dumps_quality = first_dump_per_token()
    first_pumps_quality = first_pump_per_token()

    for name, rows in (
        ("FIRST DUMP", first_dumps_quality),
        ("FIRST PUMP", first_pumps_quality),
    ):

        good = [
            r for r in rows
            if path_quality(r) == "GOOD_PATH"
        ]

        extreme = [
            r for r in rows
            if path_quality(r) == "PRICE_OUTLIER_PATH"
        ]

        nodata = [
            r for r in rows
            if path_quality(r) == "NO_DATA"
        ]
        ...
        print(
            f"  GOOD PATH   : {len(good)}"
        )

        print(
            f"  OUTLIER     : {len(extreme)}"
        )

        print(
            f"  NO DATA     : {len(nodata)}"
        )

        print(
            f"  MAX300 MED  : {fmt(median(maxs),1)}%"
        )

        print(
            f"  MAX300 P90  : {fmt(percentile(maxs,0.90),1)}%"
        )

        print(
            f"  MAX300 P95  : {fmt(percentile(maxs,0.95),1)}%"
        )

        print(
            f"  MIN300 MED  : {fmt(median(mins),1)}%"
        )

        print(
            f"  END300 MED  : {fmt(median(ends),1)}%"
        )
