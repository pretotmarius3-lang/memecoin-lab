#!/usr/bin/env python3
"""MEMECOIN LAB — ANIMATED RESEARCH COCKPIT V7.5.1
...
async function go(){try{let d=await(await fetch('/api',{cache:'no-store'})).json();clock.textContent=new Date(d.now*1000).toLocaleTimeString();hero.innerHTML=d.strategies.map(node).join('');factory.innerHTML=d.factory.map(x=>`<div class="cand"><span class="rank">#${String(x.rank).padStart(2,'0')}</span> <b>${E(x.status)}</b><br><span class="cyan">${E(x.feature)}</span><br><span class="sub">${E(x.family)} · score ${F(x.robust_score)} · HO ${x.holdout_n} · PF ${F(x.holdout_pf)}</span></div>`).join('');let p=d.portfolio[0],h='';if(p)h=`<div class="portfolioHero"><b>${E(p.verdict)}</b><div class="pgrid"><div><div class="ml">R64 RETURN / TRUE DD</div><div class="pbig green">${F(p.r64_return)}% / ${F(p.r64_true_dd)}%</div></div><div><div class="ml">50/50 RETURN / TRUE DD</div><div class="pbig ${p.portfolio_return>=p.r64_return?'green':'amber'}">${F(p.portfolio_return)}% / ${F(p.portfolio_true_dd)}%</div></div><div><div class="ml">MARGINAL RETURN</div><div class="pbig ${p.marginal_return_vs_r64>=0?'green':'red'}">${F(p.marginal_return_vs_r64)}pp</div></div><div><div class="ml">TEMPORAL PAIRS / OVERLAP</div><div class="pbig cyan">${p.paired_temporal} / ${P(p.overlap_rate)}</div></div></div><div class="sub">corr=${F(p.return_corr,3)} · both loss=${P(p.both_loss_rate)} · R64 DONE=${p.r64_done} · WALLET DONE=${p.wallet_done}</div></div>`;h+=d.decay.slice(0,6).map(x=>`<div class="event">${E(x.label||x.strategy||x.strategy_id||'DECAY')} · <span class="amber">${E(x.state||x.status||x.verdict||'WATCH')}</span> · DONE ${E(x.done||'—')}</div>`).join('');status.innerHTML=h||'<div class="event">Waiting for V7.2/V7.2.1 data.</div>';journal.innerHTML=d.journal.map(x=>`<div class="event"><span class="sub">${new Date(x.created_at*1000).toLocaleTimeString()}</span> · <b>${E(x.label)}</b> · <span class="cyan">${E(x.conclusion)}</span><br>${E(x.action)}</div>`).join('')}catch(e){clock.textContent='BUS ERROR '+e}setTimeout(go,1800)}go();
</script></body></html>'''
...
if __name__=='__main__':print(f'MEMECOIN LAB V7.5.1 cockpit http://{HOST}:{PORT}',flush=True);ThreadingHTTPServer((HOST,PORT),H).serve_forever()
